...
def implicit_average_cost_deg(delta_t, deg):
    # Create mesh
...
deg_list = [32, 48, 64, 80, 96, 102, 128]
time_per_round_implicit = []
for deg in deg_list:
    print('Is calculating for delta_t = ' + str(delta_t) + " deg = " + str(deg))
    per_step_cost, total_cost = implicit_average_cost_deg(delta_t, deg)
    time_per_round_implicit.append(per_step_cost)

plt.plot(deg_list, time_per_round_implicit)
legend_list = []
for i in deg_list:
    legend_list.append('implicit_deg of '+str(i))
