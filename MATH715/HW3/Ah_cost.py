...
def explicit_average_cost(delta_t):
    # define trial space
    u = TrialFunction(V_h)
...
def implicit_average_cost(delta_t):
    # define trial space
    u = TrialFunction(V_h)
...
delta_t_list = [0.005, 0.01, 0.02, 0.025, 0.04]

t_list = []
time_per_round_explicit = []
time_total_explicit = []
time_per_round_implicit = []
time_total_implicit = []
for t in delta_t_list:
    print('Is calculating for delta_t = ' + str(t))
    t_list.append(t)
    per_step_cost, total_cost = explicit_average_cost(t)
    time_per_round_explicit.append(per_step_cost)
    time_total_explicit.append(total_cost)

    per_step_cost, total_cost = implicit_average_cost(t)
    time_per_round_implicit.append(per_step_cost)
    time_total_implicit.append(total_cost)

plt.plot(t_list, time_per_round_explicit)
plt.plot(t_list, time_per_round_implicit)
plt.legend(['explicit', 'implicit'])
plt.title('cost per step')
plt.show()
plt.close()

plt.plot(t_list, time_total_explicit)
plt.plot(t_list, time_total_implicit)
plt.legend(['explicit', 'implicit'])
plt.title('total cost for steps = ' + str(400))
plt.show()
plt.close()
